...
let positions = [];
let apiUrl = "https://wxs.ign.fr/choisirgeoportail/itineraire/rest/route.json";
let layer_routing = "";
let layer_buffer = "";

//gestion des markers (ajouter et supprimer par leur _leaflet_id)

let markerGroup = L.layerGroup(); addOjectOnMap(markerGroup, map);
let layerIds = [];
let layer_geocodeIds = [];
let layer_geodecodeIds = [];
...
function affichage_points() {
  ...
  positions.forEach(function (position) {
    p += 1;
    let tepoints = ` Points n° ${p}:  Lng : ${position.lng}  et  Lat : ${position.lat}`;
...
function centre_routing() {
  affichage_points();
  let pos = positions.slice(-2);
  let indexMax = pos.length - 1;
...
function buffer_turf(pt, options, r) {
  if (positions.length > 0) {
    message_positions();
    let buffered = turf.buffer(pt, r, options);
    let layer_buffer = L.geoJSON(buffered);
...
function routing_geoportail() {
  ...
  if (positions.length > 1) {
    ...
    let indexMax = positions.length - 1;
    let origin = `${positions[0].lng},${positions[0].lat}`;
    let destination = `${positions[indexMax].lng},${positions[indexMax].lat}`;
    let waypoints = "";
    if (positions.length > 2) {
      for (let i = 1; i < indexMax; i++) {
    ...
    let url = `${apiUrl}?origin=${origin}&destination=${destination}`;
    ...
    fetch(url)
      ...
      .then(function (data) {
        let wkt = data["geometryWkt"];
        /**
         * TODO 2eme technique pour tracer l'itineaire sur la carte
         */
        let geojson = omnivore.wkt.parse(wkt);
 ...
 */
let position_map = (lat, lng, icon, message, layer, zoom) => {
  let pop = `<h5>[${lat} , ${lng}]</h5>`;
...
async function geoCode(adresse) {
  ...
  if (state == true) {
    ...
    let adresse_modif = etl_adresse(adresse);
...
document
  ...
  .addEventListener("click", () => {
    let adresse = document.querySelector("#geocodage_adresse").value;
    if (adresse != "") {
      document
        .querySelector("#calcul_geocodage_adresse")
        .classList.remove("btn-primary");
...
document.querySelector("#calcul_geodecodage").addEventListener("click", () => {
  let lat = document.querySelector("#lat_geodecodage").value;
  let lng = document.querySelector("#lng_geodecodage").value;
